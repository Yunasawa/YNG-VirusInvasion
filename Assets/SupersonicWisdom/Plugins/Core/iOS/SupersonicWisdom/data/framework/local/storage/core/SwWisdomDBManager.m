...
- (NSInteger)insertInto:(NSString *)table forColumns:(NSString *)columns withValues:(NSDictionary *)values {
    @synchronized (dbLock) {
        if (!isDBCreated) {
        ...
        // Create placeholders based on the number of values
        ...
        NSString *sql = [NSString stringWithFormat:@"INSERT INTO %@ (%@) VALUES (%@)", table, columns, placeholders];
            ...
            if (result == SQLITE_OK) {
                for (NSUInteger i = 0; i < keys.count; i++) {
                    ...
                    if ([value isKindOfClass:[NSString class]]) {
                        sqlite3_bind_text(statement, (int)(i + 1), [value UTF8String], -1, SQLITE_TRANSIENT);
                    } else if ([value isKindOfClass:[NSNumber class]]) {
                        if (strcmp([value objCType], @encode(int)) == 0) {
                            sqlite3_bind_int(statement, (int)(i + 1), [value intValue]);
                        } else {
                            sqlite3_bind_double(statement, (int)(i + 1), [value doubleValue]);
                        }
                    } else if ([value isKindOfClass:[NSDictionary class]]) {
                        NSData *jsonData = [NSJSONSerialization dataWithJSONObject:value options:0 error:nil];
                        NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
                        sqlite3_bind_text(statement, (int)(i + 1), [jsonString UTF8String], -1, SQLITE_TRANSIENT);
                    } else {
                        sqlite3_bind_null(statement, (int)(i + 1));
                    }
                }
                ...
                if (result == SQLITE_DONE) {
                    ...
                    [SWSDKLogger logMessage:@"SQL Success: Row ID: %ld", (long)rowId];
                } else {
                    [SWSDKLogger logMessage:@"SQL Error: %s", sqlite3_errmsg(database)];
                }
            } else {
                [SWSDKLogger logMessage:@"SQL Prepare Error: %s", sqlite3_errmsg(database)];
            }
            sqlite3_finalize(statement);
        } else {
            [SWSDKLogger logMessage:@"DB Open Error: %s", sqlite3_errmsg(database)];
        }
