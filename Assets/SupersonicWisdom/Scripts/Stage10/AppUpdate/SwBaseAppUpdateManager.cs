...
namespace SupersonicWisdomSDK
{
    internal abstract class SwBaseAppUpdateManager : ISwAppUpdateManager, ISwTacSystemListener
    {
        ...
        {
            SwInfra.CoroutineService.StartCoroutine(TryToUpdateApp());
        ...
        public void OnConfigResolved(ISwCoreInternalConfig internalConfig, ISwConfigManagerState state, ISwConfigAccessor swConfigAccessor, ESwConfigSource source)
        {
        ...
        public virtual IEnumerator TryToUpdateApp()
        {
            ...
            yield return CheckForUpdateComplete();
            
            if (IsWisdomUpdatePopupEnabled)
            {
                yield return TryPromptCustomAppUpdate();
                yield break;
            }
        ...
        private IEnumerator CheckForUpdateComplete()
        {
            yield return new WaitUntil(() => _appUpdateCheckComplete);
        }
        
        protected virtual IEnumerator TryPromptCustomAppUpdate()
        {
        ...
        {
            SwInfra.CoroutineService.StartCoroutineWithCallback(_appUpdateValidator.CheckForUpdate, () =>
            {
        ...
        {
            if (Enum.TryParse(nativePopupType, true, out ESwAppUpdateNativePopupType nativePopupSubType))
            {
        ...
        {
            if (Enum.TryParse(customUpdatePopupType, true, out ESwAppUpdatePopupType customPopupType))
            {
