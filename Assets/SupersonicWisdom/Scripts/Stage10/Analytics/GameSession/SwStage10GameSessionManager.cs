...
namespace SupersonicWisdomSDK
{
    internal class SwStage10GameSessionManager : ISwStage10ConfigListener, ISwScriptLifecycleListener, ISwReadyEventListener, ISwLocalConfigProvider, ISwTrackerDataProvider
    {
        ...
        private const string RETURN_FROM_BACKGROUND = "return_from_background";
        protected const string BACKGROUND = "background";
        private const string CLOSED = "closed";
        private const string SESSION_OPEN = "session_open";
        private const string GAME_SESSION_TRIGGER_FORMAT = "game_session_{0}";
        ...
        protected SwCrossSessionTimer _nettoPlaytimeTimer;
        ...
        protected virtual void ReportSession(string flag, ESessionStatus sessionStatus, string reason)
        {
            NotifyThirdParties(flag, sessionStatus, reason);
        ...
        protected virtual void NotifyThirdParties(string flag, ESessionStatus sessionStatus, string reason) { }
        ...
        private void NotifySessionListeners(string flag, ESessionStatus sessionStatus)
        {
            ...
            {
                SwInfra.TacSystem.InternalFireTriggers(GAME_SESSION_TRIGGER_FORMAT.Format(sessionStatus));
            }
        }
        ...
        private void ReportAndResetSession(string flag, string reason)
        {
            LogSession("Game session ended", 0, flag);
            ReportSession(flag, ESessionStatus.Ended, reason);
            RestartTimers();
        ...
        private void OpenSession(string reason)
        {
            GenerateAndSaveNewSessionId();
            RestartTimers();
			ReportSession(REPORT_SESSION_FLAG_WISDOM, ESessionStatus.Started, reason);
        }
        
        protected void CloseSession(string flag, string reason)
        {
            ReportAndResetSession(flag, reason);
        }
        
        protected virtual void AttemptToCloseSession(string flag, string reason)
        {
            CloseSession(flag, reason);
        }
        ...
        public void OnSwReady()
        {
            ...
            {
                ReportSession(REPORT_SESSION_FLAG_USER, ESessionStatus.Ended, CLOSED);
                RestartTimers();
            }

            OpenSession(SESSION_OPEN);
        }
        ...
        public void OnApplicationPause(bool isPaused)
        {
            ...
            else
            {
                ...
                if (_lastPauseTime.HasValue)
                {
                    ...
                    {
                        AttemptToCloseSession(REPORT_SESSION_FLAG_WISDOM, RETURN_FROM_BACKGROUND);
                    }
                }
