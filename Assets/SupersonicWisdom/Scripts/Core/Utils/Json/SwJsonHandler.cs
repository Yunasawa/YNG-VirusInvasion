...
namespace SupersonicWisdomSDK
{
    internal class SwJsonHandler
    {
        ...
        public object DeserializeObject(string json, Type type)
        {
            try
            {
                return !json.SwIsNullOrEmpty() ? JsonConvert.DeserializeObject(json, type) : null;
            }
            catch (JsonException ex)
            {
                SwInfra.Logger.LogException(ex, EWisdomLogType.Json,$"JSON {ErrorType.Deserializing} error: .\n objectType: {type}\n message: {ex.Message}\n");
                return null;
            }
        }
