...
namespace SupersonicWisdomSDK
{
    internal class SwParsingUtils
    {
        ...
        internal T GetFirstNonDefault<T>(T defaultVal, Func<string, T> valueSelector, params string[] keys)
        {
            var result = keys
                .Select(valueSelector)
                .Where(value => !EqualityComparer<T>.Default.Equals(value, defaultVal))
