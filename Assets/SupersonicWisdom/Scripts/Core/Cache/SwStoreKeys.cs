namespace SupersonicWisdomSDK
{
    internal static class SwStoreKeys
    {
        #region --- Properties ---

        public static string InitialConfig = "SupersonicWisdomConfig";
        public static string CustomUuid = "SupersonicWisdomCustomUuid";
        public static string UserBucket = "SupersonicWisdomUserBucket{0}";
        public static string InstallDate = "SupersonicWisdomInstallDate";
        public static string InstallTime = "SupersonicWisdomInstallTime";
        public static string LatestSuccessfulConfigResponse = "SupersonicWisdomLatestConfig";
        public static string ActiveDay = "ActiveDay";
        public static string LastActiveDate = "LastActiveDate";
        public const string InstallSdkVersion = "InstallSdkVersion";
        public const string InstallAppVersion = "InstallAppVersion";
        public const string CurrentOrganizationAdvertisingId = "currentOrganizationAdvertisingId";
        public const string PreviousOrganizationAdvertisingId = "previousOrganizationAdvertisingId";

        #endregion
    }
}