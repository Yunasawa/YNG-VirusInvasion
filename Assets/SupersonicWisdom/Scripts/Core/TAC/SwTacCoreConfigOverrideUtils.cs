...
namespace SupersonicWisdomSDK
{
    public static class SwTacCoreConfigOverrideUtils
    {
        ...
        internal static SwTacConfig OverrideTacConfig(ISwCoreInternalConfig swCoreConfig, ISwConfigAccessor swConfigAccessor)
        {
            var tacConfig = swCoreConfig.Tac;
            ...
            var overridersPairs = swConfigAccessor.AsDictionary().GetValueWithPrefix(SwTacConfigOverrideConstants.CONFIG_PREFIX);
            var didResetConditionsDict = new HashSet<string>();
                    ...
                    try
                    {
                        switch (overrideJsonSection)
                        {
                            ...
                            case SwTacConfigOverrideConstants.CONFIG_CONDITIONS_SCHEME_NAME:
                                if (!didResetConditionsDict.Contains(id))
                                {
                                    didResetConditionsDict.Add(id);
        ...
        private static void OverrideConditions(SwActionData action, KeyValuePair<string, object> overridePair, string suffix)
        {
            ...
            try
            {
                switch (suffix)
                {
                    ...
                    {
                        action.Conditions.IsDuringLevel = bool.Parse(value.ToString());
                    ...
                    case SwTacConfigOverrideConstants.MIN_SECOND_SUFFIX_NAME:
                    {
                        action.Conditions.MinSecond = int.Parse(value.ToString());
                    ...
                    {
                        action.Conditions.MaxSecond = int.Parse(value.ToString());
                    ...
                    {
                        action.Conditions.MinRevenue = float.Parse(value.ToString());

                        break;
                    }
                    case SwTacConfigOverrideConstants.FREE_ZONE_SUFFIX_NAME:
                    {
                        action.Conditions.AdFreeZone = bool.Parse(value.ToString());

                        break;
                    }
                    case SwTacConfigOverrideConstants.IS_NO_ADS_SUFFIX_NAME:
                    {
                        action.Conditions.IsNoAds = bool.Parse(value.ToString());

                        break;
                    }
                    case SwTacConfigOverrideConstants.CONSECUTIVE_COMPLETED_LEVELS:
                    {
                        action.Conditions.ConsecutiveCompletedLevels = int.Parse(value.ToString());

                        break;
                    }
                    case SwTacConfigOverrideConstants.CONSECUTIVE_FAILED_LEVELS:
                    {
                        action.Conditions.ConsecutiveFailedLevels = int.Parse(value.ToString());

                        break;
                    }
                    default:
                    {
                        SwInfra.Logger.LogWarning(EWisdomLogType.TAC, $"Unknown key -  {overridePair.Key}");
