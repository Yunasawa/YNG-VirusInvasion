...
namespace SupersonicWisdomSDK.Editor
{
    internal static class SwMenu
    {
        ...
        #if UNITY_ANDROID
        [MenuItem(SwEditorConstants.UI.SUPERSONIC_WISDOM + "/" + SwEditorConstants.UI.ANDROID + "/Apply Network Debug", false, 202)]
        public static void ApplyNetworkSecurityConfig ()
        {
            if (SwAndroidSslApply.ApplyNetworkDebug())
            {
                SwEditorAlerts.AlertApplyNetworkDebug();
            }
            else
            {
                SwEditorAlerts.AlertCannotApplyNetworkDebug();
            }
        }
        #endif
