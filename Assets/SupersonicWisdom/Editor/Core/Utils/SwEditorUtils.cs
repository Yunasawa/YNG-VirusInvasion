...
namespace SupersonicWisdomSDK.Editor
{
    public class SwEditorUtils
    {
        ...
        private static SwSettingsFactory<SwSettings> _settingsFactory;
        private static SwSettings _settings;
        ...
        public static SwSettings SwSettings
        {
            get
            {
                ...
                {
                    _settingsFactory = new SwSettingsFactory<SwSettings>($"{SUPERSONIC_WISDOM_RESOURCE_DIR_NAME}/{SUPERSONIC_WISDOM_SETTINGS_ASSET_RESOURCE_FILE_NAME}");
                    _settings = _settingsFactory.GetInstance();
                }

                // Fix unity issue in post build script where Resource.Load doesn't always work on first run
                if (_settings == null && IsSettingsExists)
                {
                    var settings = AssetDatabase.LoadAssetAtPath($"{SwEditorConstants.ASSETS}/{SettingsAssetFileRelativePath}", typeof(SwSettings)) as SwSettings;
                    _settings = _settingsFactory.GetInstance(settings);
                }
        ...
        {
            get { return $"Assets/{SettingsAssetFileRelativePath}"; }
        }
        ...
        internal static bool IsUnityGameServicesRemoteConfigInstalled()
        {
            return DoesAssemblyExists(SwEditorConstants.UGS_REMOTE_CONFIG_ASSEMBLY_FULL_NAME);
        }
        ...
        {
            const string ironsourceClassName = "IronSourceDependenciesManager";
            const string ironsourceAssemblyName = "Unity.LevelPlay.Editor";
            const string ironsourceMethodName = "ShowISDependenciesManager";
            
            try
            {
                SwReflectionUtils.InvokeStaticMethod(ironsourceClassName, ironsourceAssemblyName, ironsourceMethodName);
            }
            catch (Exception ex)
            {
                SwEditorLogger.LogError("Error opening IronSource Integration Manager: " + ex.Message);
            }
        }
            ...
            try
            {
                if (!_settings)
                {
                    ...
                    if (!Directory.Exists($"{Application.dataPath}/Resources/{SUPERSONIC_WISDOM_RESOURCE_DIR_NAME}"))
                    {
                        ...
                        SwEditorLogger.Log($"SupersonicWisdom: Resources/{SUPERSONIC_WISDOM_RESOURCE_DIR_NAME} folder is required to store settings. it was created ");
                    }
                    ...
                    AssetDatabase.SaveAssets();
                    SwEditorLogger.Log("SupersonicWisdom: Settings file didn't exist and was created");
                    Selection.activeObject = asset;

                    _settingsFactory.GetInstance();
                }
            }
            catch (Exception e)
            {
