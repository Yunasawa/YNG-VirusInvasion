...
namespace SupersonicWisdomSDK.Editor
{
    public static class SwFileUtils
    {
        ...
        public static void GenerateEnum(int stageNumber, string enumName, string[] values)
        {
            var enumValues = !values.SwIsNullOrEmpty() ? string.Join(",\n        ", values) : string.Empty;

            var content = $@"#if SW_STAGE_STAGE{stageNumber}_OR_ABOVE
namespace SupersonicWisdomSDK
{{
    // This enum is being edited each time the unity analytics events window has changed - based on the configuration.
    ...
    {{
        {enumValues}
            ...
            try
            {
                File.WriteAllText(Path.Combine(SwEditorConstants.ASSETS, SwEditorConstants.BASE_FOLDER_NAME, "Scripts", $"Stage{stageNumber}", "Api", $"{enumName}.cs"), content);
            }
            catch (Exception e)
            {
        ...
        public static (T, bool) CreateScriptableObjectInstance<T>(string path) where T : ScriptableObject
        {
            ...
            if (asset == null)
            {
                ...
                AssetDatabase.CreateAsset(asset, SwEditorConstants.ASSETS_RESOURCES_PREFIX + path + SwEditorConstants.ASSETS_SUFFIX);
