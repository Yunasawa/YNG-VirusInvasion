...
namespace Obi
{
    ...
    public unsafe struct NativeMultilevelGrid<T> : IDisposable where T : unmanaged, IEquatable<T>
    {
        ...
        public struct Cell<K> where K : unmanaged, IEquatable<K>
        {
            ...
            {
                this.coords = coords;
                contents = new UnsafeList<K>(0, Allocator.Persistent);
            }
            ...
            {
                int index = IndexOf(contents, entity);
                if (index >= 0)
                {
                    RemoveAtSwapBack(ref contents, index);
                    return true;
                }
                return false;
            }
            ...
            private int IndexOf(UnsafeList<K> list, K value)
            {
            ...
            private void RemoveAtSwapBack(ref UnsafeList<K> list, int index)
            {
